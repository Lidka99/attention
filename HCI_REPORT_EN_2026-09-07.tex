...
while latency may affect the interaction.
...
\begin{enumerate}
\item Existing checkpoints were reevaluated on
...
Gating a feature-map channel $z_c$ can be written as
\begin{equation}
z'_c=g_c(x)z_c.
\end{equation}
...
a difference of $-5.8$~\pp, whereas accuracy for \emph{truck} increased from
...
transfers, found a potential Top-1 change of approximately $+11$~\pp\
...
\toprule
\SetCell[r=2]{c,m} Seed & \SetCell[c=2]{c} Top-1 accuracy [\%] &
& \SetCell[r=2]{c,m} {$\Delta$ [\pp]}
& \SetCell[r=2]{c,m} {Block cost\\{}[\%]} \\
\cmidrule{2-3}
& Early exit & Full depth & & \\
\midrule
...
SD (spread) & 0.75 & 0.62 & 0.22 & 0.17 \\
\bottomrule
\end{tblr}
\par\smallskip
\begin{minipage}{\linewidth}
\footnotesize\singlespacing
\emph{Notes:} $\Delta$ = early exit $-$ full depth;
a plus sign denotes an accuracy increase, a minus sign a decrease, and zero no change.
Full depth is the final exit of the same jointly trained model.
SD is the standard deviation across three seeds (denominator $n-1$),
not a difference or a confidence interval, so it is reported without a plus sign.
\end{minipage}
\end{table}
...
For seed 123, the held-out cost difference relative to the target was $+0.08$~\pp;
